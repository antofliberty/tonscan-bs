<template>
    <header class="header"
        ...
        v-bind:class="{ 'header--mobile-search-visible': headerSearchVisible && screenSizes.isSmallScreen }">

        <div class="header__container container">
            <ui-link class="header__logo" v-show="headerLogoVisible" v-bind:to="{ name: 'index' }" style="flex-grow: 0">
                <svg v-pre fill="none" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="header-ear-right">
                <ui-search show-loopa class="header-search"
                    v-bind:focusInputField="headerSearchVisible"
                    v-bind:placeholder="$t('header.search_placeholder')"
                    v-on:collapseMobileSearch="headerSearchVisible = false"
                    v-show="headerSearchVisible || !isAppsSection"
                    @blur="hideSearch"
                />

                <nav class="header-ear-right__buttons">
                    <icon-loopa v-show="(isAppsSection || screenSizes.isSmallScreen) && searchButtonVisible" class="header-ear-right__button header-ear-right__button--loopa"
...
export default {
  ...
  data() {
    return {
      ...
      routes: [
        { name: 'Explorer', route_name: 'index' },
        { name: 'Stats', route_name: 'stats' },
        { name: 'Apps', route_name: 'apps' },
        { name: 'Jettons', route_name: 'jetton' },
      ],
    };
  },
  methods: {
    showSearch() {
      ...
      if (this.screenSizes.isSmallScreen) {
        this.headerLogoVisible = false;
      }
    },
    hideSearch() {
      this.searchButtonVisible = true;
      this.headerSearchVisible = false;
      this.headerLogoVisible = true;
    }
  },
  computed: {
    isAppsSection() {
      return this.$route.name === 'apps' || this.$route.name === 'app';
    }
  }
};
</script>
...
.header {
    ...
    &__nav {
      ...
      .nav-item {
        ...
        font-weight: 500;
        height: 46px; // Cannot inherit, set manually
        line-height: 46px;
